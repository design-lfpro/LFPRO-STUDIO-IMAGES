---
family: essential-lips
title: Essential Lips
product_type: Batom Líquido Matte
status: novo-produto-pendente-cadastro
preferred_tracks: [T1-product-hero, T2-texture-macro]
blocked_tracks: [T5-face-proof]
handles:
  - essential-lips-beige
  - essential-lips-rose
  - essential-lips-blush
  - essential-lips-malt
  - essential-lips-mauve
  - essential-lips-clay
  - essential-lips-carmim
  - essential-lips-wine
---

# Essential Lips — DNA de família

Linha de **batom líquido matte** LF PRO, 8 tons. **Correção 2026-09-08:** o produto NÃO é gloss — é fórmula líquida de acabamento **matte/aveludado**. O líquido parece brilhante *dentro do tubo de vidro/plástico* (reflexo natural de qualquer líquido), mas **seca matte na pele — sem brilho, sem sheen espelhado nos lábios**. Nunca gerar lábios brilhantes/glossy com este produto. **Produto novo — ainda não publicado em lfpro.com.br**, catalogado a partir de fotos de produto internas (Google Drive LF PRO, pasta "FOTO PRODUTO / SITE"), sem página no site ainda. SKU, preço e claims oficiais **não existem publicamente ainda** — não inventar, aguardar cadastro Shopify.

## Escala real (proporção) — NUNCA inflar o tamanho

- Tubo **pequeno**, compacto: comprimento aprox. **9–10 cm**, diâmetro do corpo aprox. **1,8–2,2 cm** — do tamanho de um marcador de texto fino ou batom comum, NUNCA do tamanho de um frasco de perfume/base.
- Referência de escala real (produto na mão): ver creation `KLLmtUjkqp` no Magnific — aplicador/tubo close-up segurado por uma mão, mostra a proporção real do dedo vs. tubo.
- Em still solo (packshot) sem mão/rosto na cena, não há escala de referência — mas em qualquer composição com mão/modelo, o tubo deve ocupar uma fração pequena da palma, dedos claramente maiores que a largura do tubo, nunca um objeto grande sendo "abraçado" pela mão inteira.

## Packaging (lock visual) — NUNCA redesenhar

| Elemento | Lock |
|----------|------|
| **Formato do tubo** | Tubo cilíndrico esguio, fundo arredondado (bullet-shaped), plástico **fosco/translúcido levemente acinzentado** que deixa o líquido colorido visível através da parede |
| **Tampa** | Preta **glossy/piano black**, cilíndrica, ocupa ~40% do comprimento total do tubo; friso/linha metálica prateada sutil no encontro tampa-corpo |
| **Aplicador** | Não visível no packshot fechado — não inventar formato de wand/doe-foot além do que aparecer em referência |
| **Logo** | Monograma **LF** dourado estilizado (L e F interligados, geometria vazada característica da marca) centralizado no corpo do tubo, sobre o líquido |
| **Wordmark** | `LF PRO` gold, uppercase, letter-spaced, centralizado logo abaixo do monograma |
| **Cor do logo** | Dourado ~`#C9A227`–`#D4AF37`, idêntico ao padrão da marca (nunca prata/branco) |
| **Fundo packshot** | Off-white/cream `#F5F3EF`–`#FAFAF8`, sombra soft, produto centralizado vertical |

## 8 tons — cores (hex aproximados, extraídos do packshot real)

| # | Tom | Hex aprox. | Leitura visual |
|---|-----|------------|-----------------|
| 01 | **Beige** | `#E3847A` | nude rosado terracota, quente |
| 02 | **Rose** | `#C66773` | rosa envelhecido / dusty rose |
| 03 | **Blush** | `#ED7998` | rosa pink vibrante, coral-pink |
| 04 | **Malt** | `#D45567` | rosa framboesa intenso |
| 05 | **Mauve** | `#CE617C` | rosa mauve acinzentado |
| 06 | **Clay** | `#A64B51` | terracota/tijolo avermelhado |
| 07 | **Carmim** | `#B61216` | vermelho carmim vibrante |
| 08 | **Wine** | `#5C1529` | vinho profundo / bordô escuro |

- Hex extraídos por amostragem de pixel do líquido no packshot oficial (`01.png` de cada handle) — não são valores de fórmula, apenas referência de still.
- Tubo/plástico do corpo: cinza translúcido fosco, aprox. `#D8D5D2` com opacidade
- Cap preto: `#0A0A0A`–`#151515`
- Fundo hero: `#FAFAF8`

## Textura

- **Batom líquido matte.** Dentro do tubo o líquido tem sheen natural de fluido (reflexo de qualquer líquido em recipiente translúcido) — isso NÃO é o acabamento final na pele.
- **Aplicado nos lábios: matte aveludado, zero brilho, zero sheen espelhado.** Nunca renderizar "glossy lips" / "shiny lips" para este produto.
- Sem glitter chunky aparente nas fotos disponíveis
- Ainda não temos macro de textura/swatch aplicado — se surgir arquivo "TEXTURA" na pasta de origem, complementar depois

## Estúdio

| Shot | Uso V1 |
|------|--------|
| 01 packshot off-white (por tom) | **T1 lock** obrigatório |
| Composição hero grupo (múltiplos tons) | Campanha — inspiração de composição em stacked/grouped tubes, sempre com fidelidade 1:1 a cada tubo real |

## Claims (site — sem inventar)

- **Nenhum claim oficial disponível ainda** — produto sem página publicada em lfpro.com.br no momento do cadastro (2026-09-04)
- Não escrever bullets de benefício até termos body_text real da ficha/site

## Prompt anchors (EN) — família

```
LF PRO Essential Lips liquid matte lipstick tube identity lock: SMALL slim cylindrical bullet-bottom tube, approx 9-10cm tall (compact, pocket-size, NOT an oversized bottle), frosted translucent grey-tinted plastic body showing colored liquid inside, glossy piano-black cap approx 40 percent of tube length, thin subtle silver ring at cap-body seam, warm gold interlocked LF monogram centered on tube body, LF PRO gold wordmark letter-spaced below monogram, off-white #FAFAF8 seamless studio background, soft beauty lighting, photorealistic ecommerce product photo, exact shade liquid color per SKU, no redesign of tube shape or logo. If lips are shown: matte velvety finish, zero shine, zero gloss sheen — this is a liquid MATTE lipstick, never render glossy/shiny lips.
```

## Anti-patterns

- Redesenhar formato do tubo, tampa ou aplicador
- Logo prata/branco ou monograma inventado
- Inventar claims, SKU, preço ou nome de fórmula
- Trocar cor do líquido entre tons
- Full-face / lip-proof V1 (T5 bloqueado)
- Fundo sujo, madeira, neon

## Notas V1

- 8 assets (`01.png` por handle), origem: Google Drive interno LF PRO (pasta FOTO PRODUTO / SITE), fotos datadas 2026-08-21 a 2026-08-27
- Pendências para completar cadastro: SKU, preço, URL do produto, claims oficiais (body_text), textura/swatch macro
- Ver fichas individuais por tom: [[essential-lips-beige]], [[essential-lips-rose]], [[essential-lips-blush]], [[essential-lips-malt]], [[essential-lips-mauve]], [[essential-lips-clay]], [[essential-lips-carmim]], [[essential-lips-wine]]
